...
;(function($) {
  $.fn.marquee = function(speed, direction,offset,done) {
    offset = offset || 0;
    var ih=0, oh=0, timer, mq, delayrun,style;
    var i_elem = this.find(".marquee-inner");
    var o_elem = this.find(".marquee-outer");
    var c_elem = i_elem.children();
    if(direction === "left"){
         ih = o_elem.width();
         c_elem.forEach(function(_i){
            oh+=$(_i).width();
         });
    }
    var me = this;
    var transitionEnd = function(event) {
        ...
        if (event.target != i_elem[0]) { return; }
        // Clear the animation
        i_elem[0].style['-webkit-animation'] = '';
        // Clean up the DOM
        document.head.removeChild(style);
        // We don't want that listener firing for future anims
        i_elem[0].removeEventListener('animationend', transitionEnd);
        if($.isFunction(done)){
          done();
        }
        walk(ih,(-(oh+offset))+'px');
    };
    var walk = function(from, to) {
        style = document.createElement('style');
        // Generate a unique name
        var animName = 'anim' + Date.now() + Math.floor(Math.random() * 10000);
        // Build the CSS
        style.textContent = '' +
          '@-webkit-keyframes ' + animName + ' { 0% {margin-left:' + from + 'px;} 100% {margin-left:' + to + ';}}';
          // Add it to the page
          document.head.appendChild(style);

          // Listen for end of transition
          i_elem[0].addEventListener('animationend', transitionEnd);

          // Start the animation
          i_elem[0].style['-webkit-animation'] = animName + ' '+speed+'s linear infinite';
    };
    /* var clearTimer = function() {
        i_elem.trigger('animationend');
    };
    delayrun = function(){
        var _s = i_elem.style.transition.match(/\d/i);
        _s = _s[0];
        setTimeout(function(){
            walk(_s,-oh);
        },1000);
    ...
    walk(ih,(-(oh+offset))+'px');
    //this.find(".marquee-inner").on('touchstart',clearTimer); //鼠标移动上面停止跑马灯
    //this.find(".marquee-inner").on('touchend',delayrun);//鼠标移出开始跑马灯
  };
})(Zepto);